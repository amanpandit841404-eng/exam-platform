...
function generateAllExams() {
  ...
  const medicalNames = ["NEET UG","NEET PG","NEET SS","AIIMS PG","AIIMS Nursing","JIPMER PG","FMGE","Nursing Officer","NORCET","GPAT","NIPER MSc","NIPER PhD","AIAPGET","ESIC MO","ESIC SR","ICMR JRF","BHU Ayush"];
  ...
  const enggNames = ["JEE Main","JEE Advanced","GATE","BITSAT","NATA","COMEDK","MHT CET","KCET","WBJEE","AP EAMCET","TS EAMCET","BCECE","CG PET","OJEE","UPSEE","IPU CET B.Tech","JCECE","TNEA","CUET Engg","AMU Engg","JMI Engg"];
  ...
  const teachingNames = ["CTET","UPTET","DSSSB TGT","DSSSB PGT","KVS PRT","KVS TGT","KVS PGT","NVS PRT","NVS TGT","NVS PGT","HTET","BTET","MPTET","REET","APTET","MAHA TET","OTET","CGTET","Gujarat TET","UKTET","DSSSB Librarian","DSSSB Music Teacher","RPSC Sr Teacher","BPSC Teacher"];
  ...
  const defNames = ["Indian Army GD","Indian Army Clerk","Indian Army Tradesman","Indian Army NA","Indian Army NCC","Indian Army TES","Indian Navy SSR","Indian Navy AA","Indian Navy MR","Indian Navy SSC","Indian Navy Admin","Indian Navy Logistics","IAF Airmen GD","IAF Airmen Tech","IAF AFCAT","IAF Meteorologist"];
  ...
  const capfNames = ["CISF HC","CISF Constable","CISF ASI","CRPF HC","CRPF Constable","CRPF ASI","BSF HC","BSF Constable","ITBP HC","ITBP Constable","SSB SI","SSB ASI","Assam Rifles GD","Assam Rifles SI","IB Security Asst"];
  ...
  const mbaNames = ["CAT","XAT","SNAP","NMAT","CMAT","MAT","ATMA","MAH MBA CET","TISSNET","IIFT MBA"];
  ...
  const insNames = ["LIC ADO","LIC AAO","LIC Assistant","NIACL AO","NIACL Assistant","OICL AO","OICL Assistant","United India AO","United India Assistant","ICICI Prudential AM","HDFC Life AM","SBI Life AM","IRDAI Assistant","IRDAI AO","LIC HFL"];
  ...
  const schoolNames = ["JNVST Class 6","JNVST Class 9","AISSEE Class 6","AISSEE Class 9","RIMC Dehradun","AMU City School","NTSE Stage 1","NTSE Stage 2","NMMS","KVPY SA","KVPY SB","KVPY SX","SOF IMO","SOF NSO","SOF IEO","SOF IGKO","SilverZone IOM","SilverZone IOS","SilverZone IOEL","Hummingbird Olympiad","Unified Council","EduHeal Olympiad","NSTSE","IAIS"];
  ...
  const sportsNames = ["Sports Officer","Sports Coach","SAI Coach","NIS Coach","State Sports Officer","District Sports Officer"];
  ...
  const miscNames = ["CA Foundation","CA Inter","CA Final","CS Foundation","CS Executive","CS Professional","CMA Foundation","CMA Inter","CMA Final","CSEET","NIELIT CCC","NIELIT O Level","NIELIT A Level","Hotel Management NCHMCT","NITI Aayog Intern","PFRDA Asst","ISI Exam","NIELIT B Level","NIELIT C Level","CREST","IFE"];
...
export default function AdminPage() {
  ...
  async function handleGenerate() {
    ...
    addLog("🔄 Sending to server for insert...");
    setStatus("inserting");
    setProgress({ done: 0, total: allExams.length });

    try {
      // Send to API route (server-side insert - no CORS issues!)
      ...
      if (data.success) {
        addLog(`✅ Insert complete! ${data.totalInDatabase} exams in DB`);
        setExamCount(data.totalInDatabase);
        setProgress({ done: data.totalInDatabase, total: allExams.length });
      } else {
        addLog(`❌ Error: ${data.error || 'Unknown error'}`);
      }
    } catch (err) {
      addLog(`❌ API Error: ${err.message}`);
      addLog("🔄 Trying direct browser insert as fallback...");
    ...
    <div style={{maxWidth:600,margin:"50px auto",padding:"0 20px",fontFamily:"Arial"}}>
      ...
      {(status==="generating"||status==="inserting") && (
        <div style={{textAlign:"center"}}>
          ...
          <p style={{fontWeight:600,color:"#16a34a"}}>{status==="generating"?"Generating...":"Inserting via server..."}</p>
          {progress.total>0 && (
