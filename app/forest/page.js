...
export default function Forest() {
  ...
  useEffect(() => {
    document.title = "Forest Exams Center - IFS, Forest Guard, RFO, Van Rakshak | SarkariSetu India";
  ...
  return (
    <div style={{ minHeight: "100vh", background: bg, fontFamily: "sans-serif", paddingBottom: 80 }}>
      <div style={{ background: data?.hero?.gradient || "linear-gradient(135deg,#1a2e05,#65a30d)", padding: "18px 16px 16px", color: "#fff" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <div style={{ fontSize: 13, opacity: 0.9 }}>{data?.hero?.badge || "🌲 वन विभाग भर्ती"}</div>
          <h1 style={{ margin: "6px 0", fontSize: 24, fontWeight: 800 }}>{data?.hero?.title || "Forest Exams Center"}</h1>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "14px 14px 0" }}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 14 }}>
          {(data?.quickLinks || [
            ...
            ["/category/forest-environment", "🌲 All Forest Exams"],
            ["/syllabus", "📚 Syllabus"],
        ...
        <div style={{ marginBottom: 14 }}>
          <input
            ...
            placeholder="🔍 खोजें — Forest Guard, RFO, Van Rakshak, IFS..."
        ...
        {loading ? (
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(250px,1fr))", gap: 12 }}>
            {filtered.map((e) => (
              <a key={e.slug} href={`/forest/${e.slug}`} style={{ textDecoration: "none", background: cardBg, border: `1px solid ${border}`, borderRadius: 16, padding: 14, display: "block", transition: "transform 0.15s" }}>
                ...
                <div style={{ fontSize: 11.5, color: "#65a30d", marginTop: 8, fontWeight: 700 }}>Details देखें →</div>
              </a>
            ))}
          </div>
        )}
        ...
        {data?.orgs?.length > 0 && (
          <div style={{ marginTop: 22 }}>
            <h2 style={{ fontSize: 18, fontWeight: 800, color: textMain, margin: "0 0 10px" }}>🌲 वन संस्थान — Official Websites</h2>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(220px,1fr))", gap: 10 }}>
              {data.orgs.map((b) => (
                <a key={b.name} href={b.site} target="_blank" rel="noreferrer" style={{ textDecoration: "none", background: cardBg, border: `1px solid ${border}`, borderRadius: 12, padding: "11px 13px", display: "block" }}>
                  ...
                  <div style={{ fontSize: 11, color: "#65a30d", marginTop: 3 }}>{b.site.replace("https://", "")} ↗</div>
                </a>
              ))}
            </div>
          </div>
        ...
        <div style={{ marginTop: 20, background: darkMode ? "#1e293b" : "#fff7ed", border: "1px solid " + (darkMode ? "#334155" : "#fed7aa"), borderRadius: 14, padding: "14px 16px" }}>
          ...
          <div style={{ fontSize: 12.5, color: textSub, lineHeight: 1.6, marginTop: 5 }}>
            {data?.note || "सभी जानकारी अनुमानित है। फीस, तारीखें और पैटर्न बदल सकते हैं — अपने राज्य के forest department portal पर official notification ज़रूर चेक करें।"}
          </div>
        </div>

        <div style={{ textAlign: "center", color: textSub, fontSize: 11.5, marginTop: 22, lineHeight: 1.6 }}>
          🌲 SarkariSetu India — आपका Forest भर्ती का पूरा साथी<br />अगली Forest notification की तैयारी आज से शुरू करें!
        </div>
      </div>
    </div>
  );
