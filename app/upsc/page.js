...
export default function UPSC() {
  ...
  useEffect(() => {
    document.title = "UPSC Exams Center - CSE (IAS/IPS), NDA, CDS, CAPF | SarkariSetu India";
  ...
  return (
    <div style={{ minHeight: "100vh", background: bg, fontFamily: "sans-serif", paddingBottom: 80 }}>
      <div style={{ background: data?.hero?.gradient || "linear-gradient(135deg,#7f1d1d,#dc2626)", padding: "18px 16px 16px", color: "#fff" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <div style={{ fontSize: 13, opacity: 0.9 }}>{data?.hero?.badge || "🎖️ भारत की शीर्ष सिविल सेवाएँ"}</div>
          <h1 style={{ margin: "6px 0", fontSize: 24, fontWeight: 800 }}>{data?.hero?.title || "UPSC Exams Center"}</h1>
          <div style={{ fontSize: 13, opacity: 0.92, lineHeight: 1.5 }}>{data?.hero?.subtitle || "UPSC CSE (IAS/IPS), NDA, CDS, CAPF AC, ESE — सभी UPSC exams की पूरी जानकारी एक जगह। Posts, Eligibility, Exam Pattern, Syllabus और Salary।"}</div>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "14px 14px 0" }}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 14 }}>
          {(data?.quickLinks || [
            ["/results", "🏆 UPSC Results"],
            ["/admit-cards", "📄 Admit Card"],
            ["/category/upsc", "🎖️ All UPSC Exams"],
            ["/syllabus", "📚 Syllabus"],
            ["/notifications", "🔔 Updates"],
            ["https://upsc.gov.in", "🌐 Official upsc.gov.in"],
          ]).map(([href, label]) => (
        ...
        <div style={{ marginBottom: 14 }}>
          <input
            ...
            placeholder="🔍 खोजें — UPSC CSE, NDA, CDS, CAPF..."
        ...
        {loading ? (
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(250px,1fr))", gap: 12 }}>
            {filtered.map((e) => (
              <a key={e.slug} href={`/upsc/${e.slug}`} style={{ textDecoration: "none", background: cardBg, border: `1px solid ${border}`, borderRadius: 16, padding: 14, display: "block", transition: "transform 0.15s" }}>
                ...
                <div style={{ fontSize: 11.5, color: "#dc2626", marginTop: 8, fontWeight: 700 }}>Details देखें →</div>
              </a>
            ))}
          </div>
        )}
        ...
        {data?.services?.length > 0 && (
          <div style={{ marginTop: 22 }}>
            <h2 style={{ fontSize: 18, fontWeight: 800, color: textMain, margin: "0 0 10px" }}>👑 CSE से मिलने वाली प्रमुख सेवाएँ</h2>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(220px,1fr))", gap: 10 }}>
              {data.services.map((b) => (
                <div key={b.name} style={{ background: cardBg, border: `1px solid ${border}`, borderRadius: 12, padding: "11px 13px", display: "block" }}>
                  <div style={{ fontSize: 13, fontWeight: 700, color: textMain }}>{b.name}</div>
                  <div style={{ fontSize: 11, color: "#dc2626", marginTop: 3 }}>{b.sub}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <div style={{ textAlign: "center", color: textSub, fontSize: 11.5, marginTop: 22, lineHeight: 1.6 }}>
          🎖️ SarkariSetu India — UPSC aspirants का पूरा साथी<br />अगली UPSC notification की तैयारी आज से शुरू करें!
        </div>
      </div>
    </div>
  );
