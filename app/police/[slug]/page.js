...
export default function PoliceDetail() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const res = await fetch("/police-data/exams.json");
  ...
  if (!loading && !exam) {
    return (
      <div style={{ minHeight: "100vh", background: bg, fontFamily: "sans-serif", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 30 }}>
        ...
        <a href="/police" style={{ marginTop: 14, color: "#3b82f6", fontWeight: 700, textDecoration: "none", fontSize: 14 }}>← Police Center पर वापस जाएँ</a>
      </div>
    );
  }
  ...
  return (
    <div style={{ minHeight: "100vh", background: bg, fontFamily: "sans-serif", paddingBottom: 80 }}>
      {/* Hero */}
      <div style={{ background: "linear-gradient(135deg,#0f172a,#3b82f6)", padding: "18px 16px 44px", color: "#fff" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <a href="/police" style={{ color: "#fed7aa", fontSize: 13, textDecoration: "none", fontWeight: 700 }}>← Police Exams Center</a>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "0 14px" }}>
        ...
        <div style={{ marginTop: -26, background: cardBg, border: `1px solid ${border}`, borderRadius: 16, padding: "14px 16px", boxShadow: darkMode ? "none" : "0 4px 14px rgba(0,0,0,0.06)" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginTop: 12 }}>
            <div style={{ background: darkMode ? "#334155" : "#eff6ff", borderRadius: 10, padding: "10px 12px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {exam.examPattern.map((s, i) => (
              <div key={s.stage} style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
                <div style={{ minWidth: 26, height: 26, borderRadius: "50%", background: "linear-gradient(135deg,#0f172a,#3b82f6)", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12.5, fontWeight: 800, marginTop: 1 }}>{i + 1}</div>
                <div style={{ flex: 1 }}>
        ...
        <Section title="📚 Syllabus Topics" darkMode={darkMode} border={border} cardBg={cardBg} textMain={textMain} textSub={textSub}>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {exam.syllabus.map((s) => (
              <span key={s} style={{ fontSize: 12.5, fontWeight: 700, color: textMain, background: darkMode ? "#334155" : "#eff6ff", border: `1px solid ${border}`, borderRadius: 999, padding: "7px 13px" }}>{s}</span>
            ))}
          </div>
        </Section>
        ...
        <Section title="🪜 Selection Process" darkMode={darkMode} border={border} cardBg={cardBg} textMain={textMain} textSub={textSub}>
          <div style={{ fontSize: 13, fontWeight: 700, color: "#3b82f6", lineHeight: 1.7 }}>{exam.selectionProcess}</div>
        </Section>

        {/* Official website */}
        <div style={{ margin: "14px 0", textAlign: "center" }}>
          <a href={exam.officialWebsite} target="_blank" rel="noreferrer"
            style={{ display: "inline-block", textDecoration: "none", background: "linear-gradient(135deg,#0f172a,#3b82f6)", color: "#fff", fontWeight: 800, fontSize: 15, padding: "13px 28px", borderRadius: 999 }}>
        ...
        {related.length > 0 && (
          <div style={{ marginTop: 22 }}>
            <h2 style={{ fontSize: 16, fontWeight: 800, color: textMain, margin: "0 0 10px" }}>👮 और Police Exams</h2>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(160px,1fr))", gap: 10 }}>
              {related.map((r) => (
                <a key={r.slug} href={`/police/${r.slug}`} style={{ textDecoration: "none", background: cardBg, border: `1px solid ${border}`, borderRadius: 12, padding: "12px", display: "block" }}>
        ...
        <div style={{ textAlign: "center", color: textSub, fontSize: 11.5, marginTop: 22, lineHeight: 1.6 }}>
          👮 SarkariSetu India — आपका Police भर्ती का पूरा साथी
        </div>
      </div>
    </div>
  );
