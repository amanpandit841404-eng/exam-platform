...
export default function Teaching() {
  ...
  useEffect(() => {
    document.title = "Teaching Exams Center - CTET, UPTET, REET, KVS, DSSSB | SarkariSetu India";
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "14px 14px 0" }}>
        ...
        <div style={{ marginBottom: 14 }}>
          <input
            ...
            placeholder="🔍 खोजें — CTET, UPTET, REET, KVS, DSSSB..."
        ...
        {loading ? (
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(250px,1fr))", gap: 12 }}>
            {filtered.map((e) => (
              <a key={e.slug} href={`/teaching/${e.slug}`} style={{ textDecoration: "none", background: cardBg, border: `1px solid ${border}`, borderRadius: 16, padding: 14, display: "block", transition: "transform 0.15s" }}>
                ...
                <div style={{ fontSize: 11.5, color: "#db2777", marginTop: 8, fontWeight: 700 }}>Details देखें →</div>
              </a>
            ))}
          </div>
        )}
        ...
        {data?.orgs?.length > 0 && (
          <div style={{ marginTop: 22 }}>
            <h2 style={{ fontSize: 18, fontWeight: 800, color: textMain, margin: "0 0 10px" }}>🏛️ शिक्षा संगठन — Official Websites</h2>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(220px,1fr))", gap: 10 }}>
              {data.orgs.map((b) => (
                <a key={b.name} href={b.site} target="_blank" rel="noreferrer" style={{ textDecoration: "none", background: cardBg, border: `1px solid ${border}`, borderRadius: 12, padding: "11px 13px", display: "block" }}>
                  ...
                  <div style={{ fontSize: 11, color: "#db2777", marginTop: 3 }}>{b.site.replace("https://", "")} ↗</div>
                </a>
              ))}
            </div>
          </div>
        ...
        <div style={{ textAlign: "center", color: textSub, fontSize: 11.5, marginTop: 22, lineHeight: 1.6 }}>
          🎓 SarkariSetu India — आपका Teaching भर्ती का पूरा साथी<br />अगली Teaching notification की तैयारी आज से शुरू करें!
        </div>
      </div>
    </div>
  );
