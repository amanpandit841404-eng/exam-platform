...
export default function Insurance() {
  ...
  useEffect(() => {
    document.title = "Insurance Exams Center - LIC ADO, NIACL, NICL, IRDAI | SarkariSetu India";
  ...
  return (
    <div style={{ minHeight: "100vh", background: bg, fontFamily: "sans-serif", paddingBottom: 80 }}>
      <div style={{ background: data?.hero?.gradient || "linear-gradient(135deg,#4c1d95,#8b5cf6)", padding: "18px 16px 16px", color: "#fff" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <div style={{ fontSize: 13, opacity: 0.9 }}>{data?.hero?.badge || "🛡️ बीमा कंपनी भर्ती"}</div>
          <h1 style={{ margin: "6px 0", fontSize: 24, fontWeight: 800 }}>{data?.hero?.title || "Insurance Exams Center"}</h1>
          <div style={{ fontSize: 13, opacity: 0.92, lineHeight: 1.5 }}>{data?.hero?.subtitle || "LIC ADO, NIACL AO, NICL AO, UIIC, IRDAI, GIC — सभी insurance exams की पूरी जानकारी एक जगह। Posts, Eligibility, Exam Pattern, Salary।"}</div>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "14px 14px 0" }}>
        ...
        <div style={{ marginBottom: 14 }}>
          <input
            ...
            placeholder="🔍 खोजें — LIC ADO, NIACL, NICL, IRDAI..."
        ...
        {loading ? (
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(250px,1fr))", gap: 12 }}>
            {filtered.map((e) => (
              <a key={e.slug} href={`/insurance/${e.slug}`} style={{ textDecoration: "none", background: cardBg, border: `1px solid ${border}`, borderRadius: 16, padding: 14, display: "block", transition: "transform 0.15s" }}>
                ...
                <div style={{ fontSize: 11.5, color: "#8b5cf6", marginTop: 8, fontWeight: 700 }}>Details देखें →</div>
              </a>
            ))}
          </div>
        )}
        ...
        {data?.insurers?.length > 0 && (
          <div style={{ marginTop: 22 }}>
            <h2 style={{ fontSize: 18, fontWeight: 800, color: textMain, margin: "0 0 10px" }}>🏢 बीमा कंपनियाँ — Official Websites</h2>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(220px,1fr))", gap: 10 }}>
              {data.insurers.map((b) => (
                <a key={b.name} href={b.site} target="_blank" rel="noreferrer" style={{ textDecoration: "none", background: cardBg, border: `1px solid ${border}`, borderRadius: 12, padding: "11px 13px", display: "block" }}>
                  ...
                  <div style={{ fontSize: 11, color: "#8b5cf6", marginTop: 3 }}>{b.site.replace("https://", "")} ↗</div>
                </a>
              ))}
            </div>
          </div>
        ...
        <div style={{ textAlign: "center", color: textSub, fontSize: 11.5, marginTop: 22, lineHeight: 1.6 }}>
          🛡️ SarkariSetu India — आपका Insurance भर्ती का पूरा साथी<br />अगली Insurance notification की तैयारी आज से शुरू करें!
        </div>
      </div>
    </div>
  );
